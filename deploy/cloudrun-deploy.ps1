...
function Deploy-Service([string]$name, [string]$dockerfile, [string]$min, [string]$max) {
  & $gcloud run deploy $name `
    ...
    --set-secrets "SEISMIK_WEBHOOK_HMAC_SECRET=seismik-webhook:latest,SEISMIK_DEVICE_API_KEY=seismik-device:latest,SEISMIK_CROWD_MASTER_SECRET=seismik-crowd:latest,SEISMIK_CONSUMER_API_KEY=seismik-consumer:latest" `
